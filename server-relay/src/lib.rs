...
lazy_static! {
    ...
    static ref DATA: Arc<Mutex<VecDeque<SysinfoOut>>> = Arc::new(Mutex::new(
        VecDeque::with_capacity(CONFIG.servers.len() * 720)
    ));
}
    ...
    tokio::spawn(async move {
        ...
        loop {
            for server in &*CONFIG.servers {
                match client
                    .post(format!("http://{server}/sysinfo"))
                    .json("{}")
                    .send()
                    .await
                {
                    Ok(v) => {
                        DATA.lock().await.push_back(format_response(
                            v.json::<Sysinfo>().await.unwrap_or_default(),
                            server.to_owned(),
                        ));
                    }
                    Err(_) => continue,
    ...
    for server in &*CONFIG.servers {
        pings.push(
                ...
                .await
            {
                Ok(v) => format!(
                    "{{\"{server}\":{:#?}}}",
                    v.json::<Ping>().await.unwrap_or_default().ping
                ),
                Err(_) => continue,
            },
        );
